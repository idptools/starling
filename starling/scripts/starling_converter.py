...
def starling2xtc():
    ...
    parser = ArgumentParser(description="Convert .starling ensembles to a PDB topology file and XTC trajectory")
...
def starling2info():
    ...
    print(f"DDPM_WEIGHTS_PATH           : {E._Ensemble__metadata['DEFAULT_DDPM_WEIGHTS_PATH']}")
    print(f"ENCODER_WEIGHTS_PATH        : {E._Ensemble__metadata['DEFAULT_ENCODER_WEIGHTS_PATH']}")    
...
def numpy2starling():    
    ...
    # Initialize the argument parser
    parser = ArgumentParser(description="Convert nd.numpy array (generated by STARLING) files to a .starling file. Note this is ONLY to support backwards compatiblity for ensembles generated previously")

    # Add command-line arguments corresponding to the parameters of the generate function
    parser.add_argument('input_file', type=str, help="Input starling file", default=None)
    parser.add_argument('-s', '--sequence', type=str, default=None, help="Amino acid sequence of the protein; this is a required argument")
    parser.add_argument('-o', '--output', type=str, default=".", help="Directory and/or filename to save output (default: '.')")
    ...
    parser.add_argument('--build-structures', action='store_true', default=False, help="If set then the .starling file will include a reconstructred ensemble unless an XTC ensemble is passed in which case this is ignored.")
